...
class FrechetMean(torch.autograd.Function):
    ...
    def forward(ctx, x, w, K):
        mean = frechet_ball_forward(
            x, w, K, rtol=_TOLEPS[x.dtype], atol=_TOLEPS[x.dtype]
        )
...
def frechet_mean(
    ...
    return FrechetMean.apply(x, w, -ball.c)
...
def frechet_ball_forward(
    ...
    for _ in range(max_iter):
        ...
        alphas = l_prime(
            -K * xmu_ss / ((1 + K * x_ss) * (1 + K * mu_ss.unsqueeze(-1)))
        ) / (1 + K * x_ss)
        ...
        eta = (a - K * c - ((a - K * c).pow(2) + 4 * K * b_ss).sqrt()) / (
            2 * (-K) * b_ss
        )
...
def d2arcosh(x):
    ...
    x = torch.where(
        ~cond, 2 / (x**2 - 1) - 2 * x * arcosh(x) / ((x**2 - 1) ** (3 / 2)), x
    )
    return x
...
def inverse_hessian(
    ...
    third_term = third_const.reshape(*third_const.shape, 1, 1) * torch.eye(
        y.shape[-1]
    ).to(X).reshape((1,) * len(third_const.shape) + (y.shape[-1], y.shape[-1]))
